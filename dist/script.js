...
/* harmony import */ var _modules_drop__WEBPACK_IMPORTED_MODULE_12__ = __webpack_require__(/*! ./modules/drop */ "./src/js/modules/drop");
...
const accordion = triggersSelector => {
  ...
  }); //      blocks = document.querySelectorAll(itemsSlectors);
...
const calc = (size, material, options, promocode, result) => {
  ...
  const calcFunc = () => {
    ...
    if (sizeBlock.value == '' || materialBlock == '') {
      resultBlock.textContent = "Пожалуйста, выберите размер иматериал картины";
...
/* harmony default export */ __webpack_exports__["default"] = (filter); // подумать над оптимизацией кода
...
const forms = () => {
  const form = document.querySelectorAll('form'),
        ...
        upload = document.querySelectorAll('[name="upload"]'); // сообщения после отправки формы
...
const modals = () => {
  ...
  function calcScroll() {
    ...
    div.remove();
    return scrollWidth;
  } // отслеживание пролистывания страницы и окно с подарком
...
const showMoreStyles = (trigger, wrapper) => {
  ...
  btn.addEventListener('click', function () {
    ...
    this.remove();
  }); // формирование карточек на основе ответа от сервера
...
const sliders = (slides, dir, prev, next) => {
  ...
  function showSlides(n) {
    if (n > items.length) {
      // если текущее состояние больше чем ко-во слайдов
